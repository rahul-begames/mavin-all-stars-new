...
public class FortuneWheelManager : MonoBehaviour
{
	...
	public int TurnCost = 3;					// How much coins user waste when turn whe wheel
	...
	private int _currentMavinAmount;		// Started coins amount. In your project it should be picked up from CoinsManager or from PlayerPrefs and so on
	...
	{
		_currentCoins = PlayerPrefs.GetInt("CoinValue", 0);
		_currentMavinAmount = PlayerPrefs.GetInt("MavinValue", 0);
	...
	private void Awake ()
	{
		...
		CurrentCoinsText.text = PlayerPrefs.GetInt("CoinValue", 0).ToString();
	...
	public void TurnWheelForFree() { TurnWheel (true);	}
	...
	private void TurnWheel (bool isFree)
	{
		...
		_previousCoinsAmount = _currentCoins;

		// Decrease money for the turn if it is not free turn
		if (!isFree) {
			_currentMavinAmount -= TurnCost;
			
			PlayerPrefs.SetInt("MavinValue", PlayerPrefs.GetInt("MavinValue") - TurnCost);
			_manager.DataUpdate();
	...
	public void TurnWheelButtonClick ()
	{
		if (_isFreeTurnAvailable) {
			...
			if (IsPaidTurnEnabled) {
				...
				if (_currentMavinAmount >= TurnCost)
				{
					//TODO : GameManager.Instance.AnalyticsTracker.PaidSpinWheel();
					TurnWheelForCoins ();
				}
			}
		}
	...
	private IEnumerator StopRouletteSFXAfterDelay(float delay)
	{
	...
	public void RewardedAdSpin()
	{
		
		//LevelPlayIntegration.Instance.ShowRewardAd();
		//TODO : PlayerPrefs.SetInt("RewardedAdNo", 1);
	}
	...
	private void ShowTurnButtons ()
	{
		...
		{			
			...
			if (!IsPaidTurnEnabled)			// If our settings allow only free turns
			{
				...
				if (_isStarted || _currentMavinAmount < TurnCost)
	...
	private void Update ()
	{
		...
		PlayerPrefs.SetInt("CoinValue", _currentCoins);
		_manager.DataUpdate();
	}
	...
	private IEnumerator UpdateCoinsAmount ()
	{
		...
		while (elapsedTime < seconds) {
			CurrentCoinsText.text = Mathf.Floor(Mathf.Lerp (_previousCoinsAmount, _currentCoins, (elapsedTime / seconds))).ToString ();
		...
		CurrentCoinsText.text = _currentCoins.ToString ();
